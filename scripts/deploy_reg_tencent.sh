...
echo "[5/8] 生成环境变量文件"
ssh ${SERVER_USER}@${SERVER_IP} "cat > ${APP_DIR}/.env << 'EOF'
...
MAIL_PASSWORD=wjbzydtsaxtaghdc
...
BACKUP_DATABASE_URL=postgresql://postgres:slxjfzvk@dbprovider.ap-northeast-1.clawcloudrun.com:38988
...
EOF
sudo systemctl daemon-reload && sudo systemctl enable --now ${SERVICE_NAME}.service"

echo "[7/8] 写入 Nginx 站点配置并重载"
ssh ${SERVER_USER}@${SERVER_IP} "sudo tee /etc/nginx/sites-available/reg > /dev/null << 'EOF'
server {
    listen 80;
